$(document).ready(() => {
  ...
  // Department Name Form Submission
  $(document).on("submit", "form.departmentNameForm", function (e) {
    e.preventDefault()

    var formData = new FormData(this)
    var form = $(this)
    var textID = form.find("input[name='textID']").val()
    var deptName = form.find("input[name='deptName']").val()
    var isNew = form.find("input[name='isNew']").val()
    var currentPage = $(".dynamic-load.active").data("file")
    ...
    console.log("Submitting department name form:", {
      textID: textID,
      deptName: deptName,
      isNew: isNew,
    })
    ...
    $.ajax({
      ...
      success: (response) => {
        console.log("Department name update response:", response)
        ...
        if (response.success) {
          ...
          successMsg.textContent = "Department name updated successfully!"
          ...
          errorMsg.textContent = response.message || "Failed to update department name"
      ...
      error: (xhr, status, error) => {
        ...
        const errorMsg = document.createElement("div")
        errorMsg.className = "fixed top-4 right-4 bg-red-500 text-white px-4 py-2 rounded-md shadow-lg z-50"
        errorMsg.textContent = "An error occurred while updating the department name"
        document.body.appendChild(errorMsg)
  ...
  // Department Image Form Submission
  $(document).on("submit", "form.departmentImageForm", function (e) {
    e.preventDefault()
    ...
    var textID = form.find("input[name='textID']").val()
    var imgIsNew = form.find("input[name='imgIsNew']").val()
    var currentPage = $(".dynamic-load.active").data("file")
    ...
    if (!fileSelected) {
      ...
      const errorMsg = document.createElement("div")
      errorMsg.className = "fixed top-4 right-4 bg-yellow-500 text-white px-4 py-2 rounded-md shadow-lg z-50"
    ...
    console.log("Submitting department image form:", {
      sectionID: sectionID,
      textID: textID,
      imgIsNew: imgIsNew,
    })
    ...
    $.ajax({
      ...
      success: (response) => {
        console.log("Department image update response:", response)
        ...
        if (response.success) {
          ...
          const successMsg = document.createElement("div")
          successMsg.className = "fixed top-4 right-4 bg-green-500 text-white px-4 py-2 rounded-md shadow-lg z-50"
          successMsg.textContent = "Department image updated successfully!"
          document.body.appendChild(successMsg)
          ...
          const errorMsg = document.createElement("div")
          errorMsg.className = "fixed top-4 right-4 bg-red-500 text-white px-4 py-2 rounded-md shadow-lg z-50"
          errorMsg.textContent = response.message || "Failed to update department image"
          document.body.appendChild(errorMsg)
      ...
      error: (xhr, status, error) => {
        ...
        const errorMsg = document.createElement("div")
        errorMsg.className = "fixed top-4 right-4 bg-red-500 text-white px-4 py-2 rounded-md shadow-lg z-50"
        errorMsg.textContent = "An error occurred while updating the department image"
        document.body.appendChild(errorMsg)
...
// Add this function to the script.js file to ensure it's globally available
// This should be added near the loadPage function

// Make sure the loadPage function is properly exposed globally
...
// Add a helper function to show toast notifications
function showPageToast(message, type = "success", duration = 3000) {
  ...
  toast.className = `bg-${type === "success" ? "green" : type === "error" ? "red" : "blue"}-500 text-white px-4 py-2 rounded-md shadow-lg mb-2`
